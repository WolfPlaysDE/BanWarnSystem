...
public class CommandBan implements CommandExecutor {
  ...
  public boolean onCommand(CommandSender cs, Command cmd, String label, String[] args) {
    ...
    if (cmd.getName().equalsIgnoreCase("permban")) {
      if (!cs.hasPermission("system.permban"))
        return true;
      if (args.length >= 2) {
        ...
        cs.sendMessage(prefix + "§7Du hast §e" + playername
            + " §4PERMANET §7von dem Server gebant!");
        BanWarnSystem.getInstance().logger.log(LogSettings.INFO, cs.getName() + " hat "
            + playername + " permanet von dem Server gebannt! Grund:\n" + reason);
        return true;
      }
    ...
    if (cmd.getName().equalsIgnoreCase("tempban")) {
      ...
      if (args.length >= 4) {
        ...
        if (unitsList.contains(unitString.toLowerCase())) {
          ...
          cs.sendMessage("§7Du hast §e" + playername + " §7f§r §c" + value + unit.getName()
              + " §7vom Server gebannt!");
          BanWarnSystem.getInstance().logger.log(LogSettings.INFO, cs.getName() + " hat "
              + playername + " temporär von dem Server gebannt! Grund:\n" + reason);
          return true;
        }
      ...
      cs.sendMessage(prefix
          + "§c/tempban <Spieler> <Zahlenwert> <Einheit(sec/min/hour/day/week)> <Grund>");
      return true;
    }
    ...
    if (cmd.getName().equalsIgnoreCase("checkban")) {
      if (!cs.hasPermission("system.check"))
        return true;
      if (args.length == 1) {
        if (args[0].equalsIgnoreCase("list")) {
          ...
          for (String allBanned : MySQLBanManager.getBannedPlayers()) {
            cs.sendMessage(prefix + "§e" + allBanned + " §7(Grund: §r"
                + MySQLBanManager.getReason(getUUID(allBanned).toString()) + "§7)");
          }
          return true;
        }
        String playername = args[0];
        ...
        cs.sendMessage(prefix + "§eGebannt: §r"
            + (MySQLBanManager.isBanned(getUUID(playername).toString()) ? "§aJa!" : "§aNein!"));
        if (!MySQLBanManager.isBanned(getUUID(playername).toString()))
          return true;
        cs.sendMessage(prefix + "§aGrund: §r"
            + MySQLBanManager.getReason(getUUID(playername).toString()));
        cs.sendMessage(prefix + "§aVerbleiben Zeit: §r"
            + MySQLBanManager.getRemaningTime(getUUID(playername).toString()));
        return true;
      }
