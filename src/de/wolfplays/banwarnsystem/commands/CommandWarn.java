...
public class CommandWarn implements CommandExecutor {
	...
	public boolean onCommand(CommandSender cs, Command cmd, String label, String[] args) {
		...
		if(cmd.getName().equalsIgnoreCase("addwarn")) {
			if(!cs.hasPermission("system.addwarn")) return true;
			if(args.length >= 2) {
				...
				if(!MySQLWarnManager.isInTabel(getUUID(playername).toString())) {
					OfflinePlayer op = BanWarnSystem.getInstance().getServer().getOfflinePlayer(getUUID(playername));
				...
				BanWarnSystem.getInstance().getServer().broadcastMessage("§e" + playername + " §7hat einen neuen Warn bekommen!");
		...
		if(cmd.getName().equalsIgnoreCase("removewarn")) {
			if(!cs.hasPermission("system.removewarn")) return true;
				if(args.length == 1) {
					String playername = args[0];
					if(getUUID(playername) == null) {
						cs.sendMessage(prefix + "§7Der Spieler §e" + playername + " §7existiert nicht!");
		...
		if(cmd.getName().equalsIgnoreCase("checkwarn")) {
			if(cs.hasPermission("system.checkwarns")) {
				if(args.length == 1) {
					...
					cs.sendMessage(prefix + "§7Warns: §e" + MySQLWarnManager.getWarns(getUUID(playername).toString()));
					return true;
				}
				...
				if(args.length == 0) {
					if(cs instanceof Player) {
						...
						cs.sendMessage(prefix + "§7Warns: §e" + MySQLWarnManager.getWarns(p.getUniqueId().toString()));
						return true;
					}
					return true;
 				}
