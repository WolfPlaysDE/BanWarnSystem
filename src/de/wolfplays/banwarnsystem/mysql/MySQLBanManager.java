...
/**
 * Created by WolfPlaysDE On 30.03.2015 at 03:16:00
 */
public class MySQLBanManager {
  ...
  public static String getRemaningTime(String uuid) {
    ...
    return "§e" + weeks + " Woche(n) " + days + " Tag(e) " + hours + " Stunde(n) " + minutes
        + " Minute(n) " + seconds + "Sekunde(n)";
  }
  ...
  public static void ban(String playername, String uuid, String reason, long seconds) {
    ...
    BanWarnSystem.getInstance().getServer().getScheduler()
        .scheduleSyncDelayedTask(BanWarnSystem.getInstance(), new Runnable() {
          @Override
          public void run() {
  ...
  private static void MySQLBan(String playername, String uuid, long end, String reason) {
    executor.execute(new Runnable() {
      ...
      public void run() {
        String qry =
            "INSERT INTO " + MySQL.TABEL_BANNEDPLAYERS
                + " (playername, uuid, end, reason) VALUES (?, ?, ?, ?)";
        try {
